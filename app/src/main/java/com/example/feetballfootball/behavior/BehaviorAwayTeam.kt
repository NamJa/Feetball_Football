...
class BehaviorAwayTeam(private var context: Context, attrs: AttributeSet) :
    ...
    override fun onDependentViewChanged(
        ...
        val ratio = (dependency.height+dependency.y) / dependency.height
        if(ratio >= 0.45) {
            child.scaleX = ratio
            child.scaleY = ratio
        }
        ...
        val marginRight = ((dependency.width - ((dependency.width / 2)/8)) - child.width).toFloat()
        child.x = marginRight + (dependency.y*0.2f)
        return false
    }
